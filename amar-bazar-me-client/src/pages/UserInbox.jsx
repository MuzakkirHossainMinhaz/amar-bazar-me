...
const MessageList = ({
  ...
  return (
    <div
      className={`w-full flex p-4 rounded-md ${active === index ? "bg-[#00000010]" : "bg-transparent"}  cursor-pointer hover:bg-[#00000010]`}
      onClick={(e) =>
        setActive(index) ||
...
const SellerInbox = ({
  ...
  return (
    <div className="w-full flex flex-col justify-between p-5">
      ...
      {/* messages */}
      <div className="px-3 h-[calc(100vh-355px)] py-3 overflow-y-scroll">
        {
          messages && messages.map((item, index) => (
            <div className={`flex w-full my-2 ${item.sender === userId ? "justify-end" : "justify-start"}`}
              ref={scrollRef}
              key={index}
            >
              {
              ...
              }
              {
                item.text !== "" && (
                  <div>
                    <div className={`w-max p-2 rounded ${item.sender === userId ? "bg-blue-500" : "bg-[#38c776]"} text-white h-min`}>
                      <p>{item.text}</p>
                    </div>
