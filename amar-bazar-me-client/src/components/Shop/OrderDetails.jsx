...
const OrderDetails = () => {
  ...
  return (
    <div className={`pt-4 pb-10 ${styles.section}`}>
      <div className="w-full flex items-center justify-between">
        ...
        <Link to="/dashboard_orders">
          <div className={`${styles.button} text-white font-semibold`}>
            Order List
          </div>
        </Link>
      </div>
...

      {
        data?.status === "Processing refund" || data?.status === "Refund Success" ? (
          ...
          >
            {
              ["Processing refund", "Refund Success"].slice(
                [
                  "Processing refund",
                  "Refund Success",
                ].indexOf(data?.status)).map((option, index) => (
      ...
      <div className={`${styles.button} !mt-7 text-white`}
