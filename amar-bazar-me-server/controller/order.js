...
router.post("/update-payment-info", catchAsyncErrors(async (req, res, next) => {
  try {
    const { trxId } = req.query;

    const result = await Order.updateMany(
      { "paymentInfo.trxId": trxId },
      { $set: { "paymentInfo.paid": true, "paymentInfo.paidAt": new Date() } }
    );

    if (result.nModified === 0) {
      return next(new ErrorHandler("No orders found!", 404));
    } else {
      res.redirect(`${process.env.FRONTEND_URL}/order/success?transactionId=${trxId}`);
    }
  } catch (error) {
    return next(new ErrorHandler(error.message, 500));
  }
...
router.get("/get-order-by-transaction/:id", catchAsyncErrors(async (req, res, next) => {
  try {
    const { id } = req.params;

    const orders = await Order.find({ "paymentInfo.trxId": id });
...
router.post("/fail-payment", catchAsyncErrors(async (req, res, next) => {
  try {
    const { trxId } = req.query;

    const result = await Order.deleteMany({ "paymentInfo.trxId": trxId });
...
router.post("/cancel-payment", catchAsyncErrors(async (req, res, next) => {
  try {
    const { trxId } = req.query;
    ...
    if (result.deletedCount === 0) {
      ...
      res.redirect(`${process.env.FRONTEND_URL}/order/canceled`);
    }
  } catch (error) {
    return next(new ErrorHandler(error.message, 500));
  }
